...
Загрузка файла идёт в **R2** (`IMAGES` binding) и публичный URL (`PUBLIC_IMAGE_BASE_URL` или `R2_PUBLIC_URL`). Если это не настроено, в форме «Ссылка» можно указать **HTTPS-ссылку на картинку** в поле обложки или отправить игру **без обложки**.
