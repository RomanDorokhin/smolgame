...
import { DEFAULT_CODING_MODEL } from '../config/models.js';
...
export function createContentGeneratorConfig(
  ...
  if (authType === AuthType.QWEN_OAUTH) {
    ...
    return {
      ...
      model: newContentGeneratorConfig.model || DEFAULT_CODING_MODEL,
  ...
  if (authType === AuthType.USE_OPENAI) {
    ...
    return {
      ...newContentGeneratorConfig,
      model: newContentGeneratorConfig?.model || process.env['OPENAI_MODEL'] || 'gpt-4o-mini', // Default to gpt-4o-mini if no model is specified
    } as ContentGeneratorConfig;
  }
