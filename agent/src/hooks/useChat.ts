  ...
  const sendMessage = useCallback(async (content: string, isHidden: boolean = false) => {
    ...
    for (const provider of providersToTry) {
      ...
      try {
        ...
        const SMART_SYSTEM_PROMPT = `Ты — Дружелюбный Геймдизайнер и Архитектор SmolGame.
Твоя цель — помочь пользователю создать хитовую мини-игру. Игры для SmolGame должны соответствовать правилам: управление касаниями (touch), только вертикальная ориентация (portrait).
...
Пожалуйста, напиши полный рабочий код игры (HTML+JS+CSS в одном файле) внутри тегов <game_prototype>...</game_prototype>.
Постарайся сделать игру максимально интересной и учесть все пожелания пользователя! Не пиши лишнего текста вне тегов кода.
` : `
Мы находимся на этапе сбора идей (Собрано ${progress.filled}/7 параметров).
Уже собрано:
${JSON.stringify(currentAnswers, null, 2)}

Твоя задача на сейчас — узнать у пользователя параметр: "${nextFieldRu}".

ПОЖАЛУЙСТА, следуй этим шагам:
1. Подумай вслух внутри тегов <thought>...</thought> перед тем как ответить.
2. Задай ровно один, короткий и понятный вопрос про параметр "${nextFieldRu}". Очень прошу, не задавай несколько вопросов сразу и не используй списки.
