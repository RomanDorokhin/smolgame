...
import type { GameSpec, GameSessionPhase } from './types';
...
// Simplified types for the orchestrator
...
export class GameFlowOrchestratorV2 {
  ...
  async generateGame(): Promise<boolean> {
    try {
      ...
      const result = await runGamePipeline(
        this.gameId,
        answers as any, // Temporary cast until types are fully aligned
        {
          onProgress: (message) => {
            this.notifyStepUpdate({
              phase: 'generating',
              status: 'in_progress',
              message,
              progress: 50, // Progress estimation
            });
          }
        }
      );
  ...
  async submitFeedback(feedback: string): Promise<boolean> {
    try {
      ...
      // Save edited game
      this.sessionManager.saveEditedGame(editResult.htmlCode, feedback, editResult.changes || 'No changes specified');
