...
import { Menu, Sparkles, Copy, Github, X, AlertCircle, Layout, MessageSquare, Play, Pencil } from "lucide-react";
...
export default function Home() {
  ...
  useEffect(() => {
    if (activeTab === "studio" && isAuthenticated) {
      loadMyGames();
    }
      ...
      <main className="flex-1 flex flex-col min-w-0 relative h-full">
        ...
        <div className="flex items-center justify-between px-4 py-3 z-30 shrink-0 border-b border-white/5 bg-[#0a0b0e]/80 backdrop-blur-md">
            <div className="flex items-center gap-2">
              ...
              {activeTab === 'chat' && currentSession.messages.length > 0 && (
                <Button
                    variant="ghost"
                    size="sm"
                    className="h-9 px-3 bg-[#13141a] border border-white/5 rounded-xl text-[9px] font-black uppercase tracking-widest text-[#a3b8d4] hover:text-white transition-all group ml-2"
                    onClick={(e) => {
            ...
            <div className="flex items-center gap-2">
              {currentSession.editTarget && activeTab === 'chat' && (
                <Button
                    variant="outline"
                    size="sm"
                    className="h-9 px-4 bg-blue-500/10 border-blue-500/20 text-blue-400 hover:bg-blue-500/20 rounded-xl text-[9px] font-black uppercase tracking-widest animate-pulse"
                    onClick={saveChanges}
            ...
            <ScrollArea className="h-full bg-[#0d0e14]">
              <div className="max-w-2xl mx-auto px-6 py-10">
                <div className="flex items-center justify-between mb-8">
                  ...
                  <Button variant="ghost" size="sm" onClick={loadMyGames} className="text-[#a3b8d4]">
                    Обновить
                  </Button>
                </div>
                    ...
                    {myGames.map((game) => (
                      <div key={game.id} className="group relative flex flex-col p-4 rounded-2xl bg-[#13141a] border border-white/5 hover:border-white/10 transition-all">
                        <div className="flex items-start justify-between gap-4 mb-4">
                          ...
                          {game.imageUrl ? (
                            <img src={game.imageUrl} className="w-16 h-16 rounded-xl object-cover border border-white/10" alt="" />
                          ) : (
